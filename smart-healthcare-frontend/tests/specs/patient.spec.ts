...
test.describe('Verify Patient Access', () => {
  test('TC-047 Verify that a patient can not open an admin page', async ({ commonPage }) => {
    await commonPage.goto(ROUTES.ADMIN_DOCTORS);

    await commonPage.verifyUrl(ROUTES.DASHBOARD);
  });

  test('TC-048 Verify that a patient can not open a doctor page', async ({ commonPage }) => {
...
test.describe('Verify Patient Appointments', () => {
  test('TC-049 Verify that the patient can book an appointment, cancel it and delete it', async ({ patientPage }) => {
  ...
  test('TC-050 Verify that the booking form does not book before a doctor and a date are picked', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_APPOINTMENTS);
  ...
  test('TC-051 Verify that a day the doctor does not work offers no times', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_APPOINTMENTS);
    await patientPage.openBookingForm();
  ...
  test('TC-052 Verify that Cancel throws away what was typed in the booking form', async ({ patientPage }) => {
    const reason = uniqueText(BOOKING_REASON);
  ...
  test('TC-053 Verify that Keep it in the ConfirmDialog keeps the appointment', async ({ patientPage }) => {
    const reason = uniqueText(BOOKING_REASON);
  ...
  test('TC-054 Verify that a time that is already booked is not offered again', async ({ patientPage }) => {
    const reason = uniqueText(BOOKING_REASON);
  ...
  test('TC-055 Verify that the appointments calendar shows a column for every day of the week', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_APPOINTMENTS);
  ...
  test('TC-056 Verify that the patient can walk to the next week and come back', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_APPOINTMENTS);
    await patientPage.showCalendarView();
  ...
  test('TC-057 Verify that the patient can open the details of an appointment', async ({ patientPage }) => {
    const reason = uniqueText(BOOKING_REASON);
  ...
  test('TC-058 Verify that the view the patient picked is still there after a reload', async ({ patientPage, page }) => {
    await patientPage.goto(ROUTES.PATIENT_APPOINTMENTS);
    await patientPage.showListView();
...
test.describe('Verify Patient Prescriptions', () => {
  test('TC-059 Verify that the patient can see the prescriptions written for him', async ({ patientPage }) => {
  ...
  test('TC-060 Verify that the patient can open the details of a prescription', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_PRESCRIPTIONS);
...
test.describe('Verify Patient Medical Record', () => {
  test('TC-061 Verify that the patient can see his own medical record', async ({ patientPage }) => {
  ...
  test('TC-062 Verify that the record can be filtered down to one kind of item', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_MEDICAL_RECORD);
  ...
  test('TC-063 Verify that the patient can open a visit from his record', async ({ patientPage }) => {
    await patientPage.goto(ROUTES.PATIENT_MEDICAL_RECORD);
    await patientPage.clickOnItem(recordFilter(RECORD_FILTERS.VISITS));
...
test.describe('Verify Patient Profile', () => {
  test('TC-064 Verify that the patient can change his phone and address', async ({ patientPage }) => {
